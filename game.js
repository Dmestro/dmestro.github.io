	...
	function(){
		...
		var Game = function(canvasId){
			...
			loadSound("shoot.wav",function(shootSound){
				...
				var tick = function(){ //используется для вызова дочерних функций, весь цикл игры

				self.update(gameSize);
				self.draw(screen,gameSize);
				requestAnimationFrame(tick); //рекурсия?!!!!
		...
		Game.prototype ={
			...
			update:function(gameSize){
				...
				console.log("count of bodies "+this.bodies.length);
		...
		Invader.prototype = {
			update: function(){
				...
				if(this.patrolX<0||this.patrolX>300){
				...
				var bullet = new Bullet({x:this.position.x-this.size.width/2-3/2, y:this.position.y+this.size.height/2+3/2},{x:Math.random()-0.5,y:2});
		...
		Player.prototype ={
			update:function(){
				...
				if(this.keyboarder.isDown(this.keyboarder.KEYS.SPACE)){
					if(this.bullets<5){
						var bullet = new Bullet({x:this.position.x+this.size.width/2-3/2, y:this.position.y-this.size.height/2-3/2},{x:0,y:-6});
						this.game.addBody(bullet);
						this.bullets++;
		...
		// 		b1.position.x-b1.width/2 >b2.position.x+b2.size.width/2 ||
		// 		b1.position.y-b1.height/2>b2.position.y+b2.size.height/2
		// 		);
		// }
		var colliding = function(b1, b2) {
        return (b1 != b2 && 
